...
seg_id="🤖 ${model}${ws_str}${branch_str}"
seg_ctx="📊 $(fmt $current_used)/$(fmt $context_size) ${bar} ${pct_int}% ${perf}"
seg_detail="⬇$(fmt $total_input) ⬆$(fmt $total_output) │ 💡残$(fmt $remaining_tokens) │ ⏳~${eta_str} │ 🔄${compress_count}回"
seg_burn="🔥 ${burn_rate_str}"
seg_usage="🕐 D:$(fmt $d_total)  🗓 W:$(fmt $w_total)  📊 M:$(fmt $m_total)"
seg_sid="🔑 ${session_id}"
